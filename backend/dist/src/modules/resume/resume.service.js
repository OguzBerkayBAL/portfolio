...
let ResumeService = class ResumeService {
    ...
    async create(createResumeDto) {
        ...
        if (existingResume) {
            throw new common_1.ConflictException(`Resume version '${createResumeDto.version}' already exists`);
        }
        if (createResumeDto.isActive !== false) {
            await this.resumeRepository.update({ isActive: true }, { isActive: false });
        }
        const resume = this.resumeRepository.create(createResumeDto);
        return await this.resumeRepository.save(resume);
    }
    async findAll(queryDto) {
        const { version, template, isActive, search, createdAfter, createdBefore, page = 1, limit = 10, sortBy = 'createdAt', sortOrder = 'DESC' } = queryDto;
        ...
        if (search) {
            queryBuilder.andWhere('(resume.personalInfo->>\'name\' ILIKE :search OR resume.personalInfo->>\'title\' ILIKE :search OR resume.personalInfo->>\'summary\' ILIKE :search)', { search: `%${search}%` });
        }
        if (createdAfter) {
    ...
    async findByVersion(version) {
        ...
        if (!resume) {
            throw new common_1.NotFoundException(`Resume version '${version}' not found`);
        }
        return resume;
    }
    async update(id, updateResumeDto) {
        ...
        if (updateResumeDto.version && updateResumeDto.version !== resume.version) {
            ...
            if (existingResume) {
                throw new common_1.ConflictException(`Resume version '${updateResumeDto.version}' already exists`);
            }
        }
        if (updateResumeDto.isActive === true) {
            await this.resumeRepository.update({ isActive: true, id: { ne: id } }, { isActive: false });
        }
    ...
    async generatePDF(id, template = 'modern') {
        throw new Error('PDF generation is temporarily disabled');
    }
    generateHTMLTemplate(resume, template) {
        switch (template) {
            case 'cyberpunk':
    ...
    generateCyberpunkTemplate(resume) {
        const { personalInfo, education, experience, skills, projects, languages, certifications } = resume;
        return `
      <!DOCTYPE html>
      <html lang="en">
    ...
    generateClassicTemplate(resume) {
        return this.generateModernTemplate(resume);
    }
};
